...
changes <- read.csv("raw/Growth Change.csv", stringsAsFactors=FALSE, colClasses=c(cbsa="character"))
...
#rename indicator
changes$measure <- ifelse(changes$indicator=="Percent Change in Number of Jobs in Firms 0-5 Years Old", "emp5u", ifelse(changes$indicator=="Percent Change in Employment", "emp", "output"))
...
json <- toJSON(list(measures=list(list(key=unbox("emp"), value=unbox("Employment")), 
                                  list(key=unbox("output"), value=unbox("Output")), 
...
writeLines(json, "json/growth_change.json")

writeLines(json2, "json/growth_map_data.json")
